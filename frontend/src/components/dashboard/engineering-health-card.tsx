...
export function EngineeringHealthCard({
  ...
  return (
    <div className="flex flex-col lg:flex-row items-stretch gap-6 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/80 shadow-sm">
      {/* Overall Score Badge */}
      <div className="flex flex-col items-center justify-center p-6 rounded-xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200/60 dark:border-zinc-800/60 min-w-[200px]">
        <div className="relative flex items-center justify-center">
          <div className="text-4xl font-extrabold tracking-tight text-zinc-900 dark:text-white">
            {overallScore}%
          </div>
        </div>
      ...
      <div className="flex-1 grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          { label: 'Sprint Health', score: sprintHealth, icon: Activity },
          { label: 'Deployment Health', score: deploymentHealth, icon: Rocket },
          { label: 'Code Quality', score: codeQuality, icon: Code2 },
          { label: 'PR Health', score: prHealth, icon: GitPullRequest },
          { label: 'Incident Health', score: incidentHealth, icon: AlertCircle },
        ].map((item) => {
          const Icon = item.icon;
          return (
            <div
              key={item.label}
              className="flex flex-col justify-between p-3.5 rounded-xl border border-zinc-200/60 dark:border-zinc-800/60 bg-zinc-50/50 dark:bg-zinc-900/30"
            >
              ...
              <div className="mt-3">
                <span className="text-[11px] font-medium text-zinc-600 dark:text-zinc-400 block truncate">{item.label}</span>
